...
def add_worker():
    ...
    try:
        ...
        r = requests.post(url = "http://%s:%s/v1/debug/auth" % (worker_ip, worker_port), timeout = 6,
...
def rename_nodes(worker_name, new_name):
    ...
    if worker is not None:
        try:
            r = requests.post(url = "http://%s:%s/v1/admin/node/rename" % (worker.ip, worker.port), timeout = 6,
        ...
        logging.error(msg)
    close_session(db)
    return flask.redirect("/admin/worker/%s" % msg)
...
def add(el_type):
    ...
    try:
        ...
        r = requests.post(url = "http://%s:%s/v1/admin/add/%s" % (worker.ip, worker.port, el_type), timeout = 6,
            json = json_args)
        if r.status_code != 200:
...
def delete(el_type, worker_name, el_name):
    ...
    try:
        ...
        r = requests.post(url = "http://%s:%s/v1/admin/delete/%s" % (worker_ip, worker_port, el_type), timeout = 6,
            json = json_args)
        if r.status_code != 200:
...
def switch_nodes(worker_name, switch_name):
    ...
    try:
        # Get the PoE status of all switch ports
        r = requests.post(url = "http://%s:%s/v1/admin/switch/nodes/%s" % (worker_ip, worker_port, switch_name), timeout = 6,
            json = json_args)
        if r.status_code == 200:
            result = r.json()
    except:
...
def turn_off(worker_name, switch_name):
    ...
    try:
        ...
        r = requests.post(url = "http://%s:%s/v1/admin/switch/turn_off/%s" % (worker_ip, worker_port, switch_name), timeout = 6,
            json = json_args)
        if r.status_code == 200:
            result = r.json()
    except:
...
def turn_on(worker_name, switch_name):
    ...
    try:
        ...
        r = requests.post(url = "http://%s:%s/v1/admin/switch/turn_on/%s" % (worker_ip, worker_port, switch_name), timeout = 6,
            json = json_args)
        if r.status_code == 200:
            result = r.json()
    except:
...
def init_detect(worker_name, switch_name):
    ...
    close_session(db)
    try:
        r = requests.post(url = "http://%s:%s/v1/admin/switch/init_detect/%s" % (worker_ip, worker_port, switch_name), timeout = 6,
            json = json_args)
        if r.status_code == 200:
            result = r.json()
    except:
...
def dhcp_conf(worker_name, switch_name):
    ...
    close_session(db)
    try:
        r = requests.post(url = "http://%s:%s/v1/admin/switch/dhcp_conf/%s" % (worker_ip, worker_port, switch_name), timeout = 6,
            json = json_args)
        if r.status_code == 200:
            result = r.json()
    except:
...
def dhcp_conf_del(worker_name, switch_name):
    ...
    close_session(db)
    try:
        r = requests.post(url = "http://%s:%s/v1/admin/switch/dhcp_conf/%s/del" % (worker_ip, worker_port, switch_name), timeout = 6,
            json = json_args)
        if r.status_code == 200:
            result = r.json()
    except:
...
def node_conf(worker_name, switch_name):
    ...
    close_session(db)
    try:
        r = requests.post(url = "http://%s:%s/v1/admin/switch/node_conf/%s" % (worker_ip, worker_port, switch_name), timeout = 6,
            json = json_args)
        if r.status_code == 200:
            result = r.json()
        else:
...
def clean_detect(worker_name):
    ...
    close_session(db)
    try:
        r = requests.post(url = "http://%s:%s/v1/admin/switch/clean_detect" % (worker_ip, worker_port), timeout = 6,
            json = json_args)
        if r.status_code == 200:
            result = r.json()
        else:
